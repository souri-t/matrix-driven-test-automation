...
ExcelとJSONは次の3シートだけを、この順で持ちます。

- `テスト対象`
- `因子と水準`
- `テストケース`

`テスト対象` の列は `テスト対象`、`根拠資料`、`参照箇所`、`参照目的`、`備考` です。`テスト対象`だけが必須で、それ以外は空欄を許可します。根拠資料が複数ある場合は1資料を1行で表し、各行にテスト対象を記載します。値は自由入力で、区分値、ハッシュ値、スキーマバージョンは管理しません。

| テスト対象 | 根拠資料 | 参照箇所 | 参照目的 | 備考 |
| --- | --- | --- | --- | --- |
| TicketService.ResolveExpected | Spec/APIプロジェクト概要.md | 2.5 | 期待結果の確認 | |
| TicketService.ResolveExpected | src/TicketService.cs | ResolveExpected | 境界条件の確認 | |
...
`テストケース` の列は `ID`、`テストの目的`、1列以上の因子列、`expected`、`memo` です。`テストの目的`は全ケースで必須です。JSONは同じ内容をworkbook形式の `sheets` 配列で表現します。

| ID | テストの目的 | Soup | NoodleAmount | expected | memo |
| --- | --- | --- | --- | --- | --- |
| TC-001 | 塩・普通盛りの食券を確認する | 塩 | 普通 | 食券1 | |

仕様起点のケースでは、`テスト対象`に仕様書パス、対象箇所、参照目的を記録し、`memo`に内部シナリオ名などのケース固有情報を記録します。

変換時は、因子列が`因子と水準`の因子順と一致すること、ケースの値が定義済み水準であること、全水準が少なくとも1ケースで使われていることを検証します。ペアワイズ、正常系・異常系、境界条件、仕様Scenarioの網羅性は設計・監査エージェントが対象コードや仕様を基に確認します。確認状態や網羅IDはExcel/JSONへ保存しません。未確定の期待結果は従来どおり`expected`を`要確認`とし、解決するまで`testcode-writer`は停止します。

逆生成では、AIがテスト本体と呼出先から実際のテスト対象を特定し、テスト名・入力・期待値・対象コードから各ケースの目的を日本語で補います。対象を一意に特定できなければ生成前に確認します。`因子と水準`は既存DataRowに実際に現れる値だけから復元し、元のテストコードから判定できない未使用水準や設計意図は補完しません。
