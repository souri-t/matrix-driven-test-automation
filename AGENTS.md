...
Excel/JSONは「テスト対象」「因子と水準」「テストケース」の3シートだけを、この順で持つ。「テスト対象」はテスト対象、根拠資料、参照箇所、参照目的、備考を持ち、テスト対象だけを必須とする。「テストケース」はID、テストの目的、各因子列、expected、memoを持ち、テストの目的を必須とする。確認状態、網羅ID、型、因子ID、水準ID、ハッシュ値、スキーマバージョンは追加しない。

網羅性は生成・監査時に検証し、結果をExcel/JSONへ管理項目として保存しない。因子列と水準の整合および全水準の使用は決定的スクリプトで検証し、ペアワイズ、正常系・異常系、境界条件、仕様Scenarioは対象コードまたは指定仕様を基にエージェントが確認する。
